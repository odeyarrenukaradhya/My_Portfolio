...
const InfiniteSpiral = ({
  ...
  const normalizedItems = useMemo(
    () =>
      items.map((item, index) =>
        typeof item === 'string'
          ? { src: item, alt: `Design ${index + 1}` }
          : { alt: `Design ${index + 1}`, ...item }
      ),
    [items]
  );
  ...
  useEffect(() => {
    ...
    const render = (time) => {
      ...
      cardRefs.current.forEach((card, index) => {
        ...
        const blur = edgeBlur * smoothstep(0.25, 1, edge);
  ...
  const cardStyle = { width: cardWidth, height: cardHeight, borderRadius: cardRadius };
  const imageStyle = {
    width: '100%',
    height: '100%',
    objectFit: imageFit,
  ...
  const itemClassName =
    'absolute left-1/2 top-1/2 block h-[var(--spiral-height)] w-[var(--spiral-width)] overflow-hidden rounded-[var(--spiral-radius)] border border-neutral-300 bg-white shadow-[0_10px_25px_rgba(0,0,0,0.12)] [backface-visibility:hidden] [transform-style:preserve-3d] [will-change:transform,opacity,filter] motion-reduce:transition-none cursor-pointer group';
    ...
    >
      <div className="absolute inset-0 [transform-style:preserve-3d]" role="list" aria-label="Infinite spiral gallery">
        {normalizedItems.map((item, index) => {
          const content = (
            <div className="relative w-full h-full overflow-hidden rounded-[var(--spiral-radius)] transition-all duration-300 ease-out group-hover:scale-130 group-hover:shadow-[0_20px_40px_rgba(0,0,0,0.2)]">
              <img
                className="absolute inset-0 block h-full w-full select-none object-center transition-transform duration-300 group-hover:scale-105"
                src={item.src}
                alt={item.alt}
